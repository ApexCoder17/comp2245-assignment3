...
window.onload = function () {
  ...
  Array.from(document.getElementById("board").children).forEach((element) =>
    element.setAttribute("class", "square hover")
  );

  // Create an array to keep track of the state of the game
  let gameState = Array.from(document.getElementById("board").children).map(
  ...
  document.getElementById("board").addEventListener("click", (event) => {
    ...
    let index = Array.from(document.getElementById("board").children).indexOf(
      event.target
    );
    console.log(index);
    ...
    if (gameState[index] === "" && winner === "") {
      ...
      if (win(gameState)) {
        ...
        document.getElementById(
          "status"
        ).innerHTML = `Congratulations! ${winner} is the Winner!`;
  ...
  document.getElementsByClassName("btn")[0].addEventListener("click", () => {
    ...
    gameState = Array.from(document.getElementById("board").children).map(
      (element) => (element.innerHTML = "")
    );
    // Reset the player
    player = "X";
